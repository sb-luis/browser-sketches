...
// Append the switches directly into the shared metrics panel.
const divider = Object.assign(document.createElement('div'), { className: 'metric-divider' });

function makeSwitch(id, label, checked = true) {
  const lbl = document.createElement('label');
  lbl.className = 'switch-row';
  const cb = Object.assign(document.createElement('input'), { type: 'checkbox', id, checked });
  const span = Object.assign(document.createElement('span'), { className: 'switch-label', textContent: label });
  lbl.append(cb, span);
  return { el: lbl, cb };
}

const { el: swBackfaceEl, cb: swBackface } = makeSwitch('sw-backface', 'backface culling');
const { el: swFrustumEl,  cb: swFrustum  } = makeSwitch('sw-frustum',  'frustum culling');
panel.append(divider, swBackfaceEl, swFrustumEl);
...
function updateCells() {
  ...
  const useBackface = swBackface.checked;
  const useFrustum  = swFrustum.checked;
  let activeCount = 0;
  ...
  for (let i = 0; i < cells.NUM_CELLS; i++) {
    ...
    if (useBackface && center.dot(_camDir) <= 0) {
      active = false;
    }

    if (active && useFrustum) {
      _sphere.center.copy(center);
      _sphere.radius = HALF_CELL_CHORD;
      if (!frustum.intersectsSphere(_sphere)) active = false;
    }
